...
const CODE_BLOCK_RESTORE_PATTERN = new RegExp(`${MARKER_STX}CCIMCB(\\d+)${MARKER_STX}`, "g");
const INLINE_CODE_RESTORE_PATTERN = new RegExp(`${MARKER_ETX}CCIMIC(\\d+)${MARKER_ETX}`, "g");
...
export function markdownToTelegramHtml(md: string): string {
  ...
  text = text.replace(CODE_BLOCK_RESTORE_PATTERN, (_, i) => codeBlocks[Number(i)] || "");
  text = text.replace(INLINE_CODE_RESTORE_PATTERN, (_, i) => inlineCodes[Number(i)] || "");
  return text;
}
